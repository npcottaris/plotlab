...
```plotlab``` is a toolbox for the automatic generation of publication quality Matlab plots. The approach followed is to define 'recipes' which override various default plotting properties of the Matlab graphics root object, so as to generate visually-engaging, publication-ready plots with minimal code, thereby avoiding the classic paradigm of repeatedly entering boiler plate code for setting various plotting options to one's liking. `Plotlab` includes several tutorials that show examples of setting default plotting parameters for a number of plot types that result in publication quality, highly legible and visually engaging figures that have a consistent appearance across computing platforms (mac and linux) and graphics formats.
